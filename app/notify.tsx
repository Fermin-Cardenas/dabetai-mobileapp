...
// Importar SegmentedControl directamente
import { SegmentedControl } from "@/components/core/navigation";
...
const Notifications = () => {
  const [activeTab, setActiveTab] = useState<"no-leidas" | "leidas">(
    "no-leidas"
  );
  ...
  const currentNotifications =
    activeTab === "no-leidas" ? noLeidasNotifications : leidasNotifications;
  ...
  return (
    ...
    >
      {/* Tabs */}
      <SegmentedControl
        options={["No leídas", "Leídas"]}
        selectedIndex={activeTab === "no-leidas" ? 0 : 1}
        onSelectionChange={(index) => {
          setActiveTab(index === 0 ? "no-leidas" : "leidas");
        }}
      />
      ...
      <CardList
        items={currentNotifications.map(notification => ({
          ...
          state: notification.isUnread ? 'unread' as const : 'read' as const
        }))}
        onItemPress={(item) => console.log('Notification pressed:', item.id)}
      />
    </AppLayout>
  );
};
