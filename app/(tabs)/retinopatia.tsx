  ...
  const fetchPrediction = async () => {
    try {
      const res = await fetch(
        "http://192.168.100.20:8000/retinopathy/predict/1"
      );
      ...
      <SafeAreaView className="flex-1 bg-[#f1f5f9]">
        ...
        >
          ...
          {nivelGeneral !== "" && (
            <RiskLevelCard
              riskLevel={
                nivelGeneral.toLowerCase() as "bajo" | "medio" | "alto"
              }
        ...
        >
          ...
          <NavButton
            title="Predicción"
            iconName="box"
            isActive={activeTab === "retinopatia"}
            onPress={() => handleNavigation("retinopatia", "/(tabs)/retinopatia")}
          />
          <NavButton
            title="Historial"
            iconName="activity"
