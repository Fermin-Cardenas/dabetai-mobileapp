...
const StatItem = ({ icon, value, label }: StatData) => (
  <View className="flex-1 mx-2 mb-6">
    <View className="flex-row items-center">
      <View className="w-16 h-16 bg-white rounded-3xl justify-center items-center mr-4 border" style={{ borderColor: '#E5E7EB' }}>
        <MaterialIcons name={icon as any} size={28} color="#6B7280" />
      </View>
      <View className="flex-1">
        <Body className="text-[#2C3E50] font-bold text-lg mb-1">{value}</Body>
